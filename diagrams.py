...
# ── 11차 배치 ─────────────────────────────────────────────────
_add("rsync-delta", "복사기 vs 교정 담당",
     "scp는 매번 전체를 다시, rsync는 비교 후 바뀐 조각만. 끊겨도 이어서 — 매일 돌리면 그게 증분 백업입니다.",
     "rsync",
     _two("rs", "scp — 복사기", [
         ("no", "상대편에 뭐가 있는지 안 봄"),
         ("no", "파일 3개 바뀌어도 10GB 전부 재전송"),
         ("no", "98%에서 끊기면 처음부터"),
         ("dot", "일회성 단순 전송엔 충분"),
     ], "rsync — 비교 후 차액만", [
         ("ok", "목록·시각 대조 → 다른 것만 전송"),
         ("ok", "재실행 = 이어서 (--partial은 파일 내부도)"),
         ("ok", "매일 cron = 증분 백업 파이프라인"),
         ("no", "끝 슬래시 = 뚜껑 열고 붓기 — -n 먼저!"),
     ], "--delete는 dry-run 없이 절대 금지 · 이관은 '미리 1차 동기화 → 전환 창에 차액만'",
        licon="doc", ricon="switch"))

_add("tag-three", "질문 세 개에 답하는 태그",
     "누구 거야, 무슨 일이야, 언제 지워 — 이 셋만 강제하면 비용 배분·고아 청소·장애 연락이 풀립니다.",
     "tag-strategy",
     _ladder("tg", [
         ("owner", "owner=infra-team", "\"누구한테 물어봐?\" — 팀 단위로", "user"),
         ("project", "project=goil-sports", "\"어디 예산이야?\" — 비용 group-by", "doc"),
         ("expire", "expire=2026-09-30", "\"언제 지워?\" — 고아 자원 백신", "shield"),
     ], "권고 말고 강제: 생성 차단 정책 · 미태그 스캔+통첩 · IaC default_tags · 표기 규칙 고정"))

_add("system-vs-user", "사장의 지침서 vs 손님의 주문",
     "모델은 시스템 지시에 더 높은 권위를 둡니다. 고정 규칙은 시스템(앞), 동적 재료는 유저(뒤)로.",
     "system-vs-user-prompt",
     _two("su", "시스템 — 지침서 (고정·권위)", [
         ("ok", "역할·불변 규칙·출력 규격"),
         ("ok", "캐시 프리픽스 — 앞에 고정 배치"),
         ("no", "검색 결과를 여기 넣기 (주입+캐시 파괴)"),
         ("no", "비밀 넣기 — 프롬프트는 금고가 아님"),
     ], "유저 — 요청과 재료 (동적)", [
         ("ok", "이번 질문 · 대화 이력"),
         ("ok", "RAG 결과·첨부 — 구분자로 감싸 '자료'로"),
         ("dot", "세션 설정 변경은 유저 턴 말고 시스템 갱신"),
         ("dot", "지침은 짧게 — 안 지켜지면 예시로"),
     ], "기준 하나: 대화가 바뀌어도 유지돼야 하는가 · 바꿨으면 골든셋 회귀 테스트",
        licon="shield", ricon="user"))


# ── 12차 배치 ─────────────────────────────────────────────────
_add("dns-cache-ladder", "DNS 조회의 4층 계단",
     "평소엔 위층 캐시에서 끝나 0ms대, TTL 만료 직후만 바닥까지 내려갔다 옵니다. hosts는 캐시가 아니라 오버라이드예요.",
     "dns-resolver-cache",
     _ladder("dc", [
         ("① /etc/hosts", "조회 전에 먼저 봄", "캐시 아님 — 정답 강제 (잊힌 한 줄 주의)", "doc"),
         ("② 로컬 스텁 캐시", "systemd-resolved (127.0.0.53)", "resolvectl flush-caches", "laptop"),
         ("③ 리졸버 캐시", "resolv.conf의 그 서버", "dig @8.8.8.8 로 판별", "server"),
         ("④ 권위 서버", "진짜 원본", "여기까지 가는 순간이 '가끔 느림'", "globe"),
     ], "실패(NXDOMAIN)도 캐시됩니다 — 새 레코드는 '만들고 나서' 조회 · 진단: getent → resolvectl → dig"))

_add("serverless-econ", "택시 vs 자가용 — 가동률이 정한다",
     "탄 만큼 내는 요금은 간헐 트래픽에 압승, 상시 트래픽에 역전. 견적은 함수가 아니라 파이프라인 전체로.",
     "serverless-economics",
     _two("se", "서버리스가 이기는 판", [
         ("ok", "간헐·예측불가 — 가동률 한 자릿수 %"),
         ("ok", "이벤트 글루 · 웹훅 · 스케줄 잡"),
         ("ok", "개발·스테이징 (밤샘 서버 소멸)"),
         ("ok", "스파이크 자동 흡수 — 스케일링 고민 제로"),
     ], "서버가 이기는 판", [
         ("no", "상시 고트래픽 — 호출 단가 × 24h 역전"),
         ("no", "장시간 배치 · 상태·연결 유지 (웹소켓)"),
         ("no", "콜드스타트가 p95를 오염시키는 API"),
         ("no", "커넥션 풀 필요 (인스턴스 폭증 × DB 한도)"),
     ], "숨은 비용: API Gateway 호출당 · NAT GB당 · 로그 수집 — \"함수 3만원, 문 앞 30만원\" 주의",
        licon="cloud", ricon="server"))

_add("embedding-choice", "임베딩 모델 선택의 4축",
     "RAG 재현율의 상한을 정하는 선택. 최종 심사는 리더보드가 아니라 내 질문 30개의 재현율@k입니다.",
     "embedding-choice",
     _ladder("ec", [
         ("① 언어", "한국어면 다국어 명시 모델", "조사·어미 변형에 강한지", "globe"),
         ("② 차원", "384 vs 1024 — 저장·속도 분모", "규모 커질 때의 다이얼", "database"),
